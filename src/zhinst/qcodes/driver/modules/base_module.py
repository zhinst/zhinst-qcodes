...
class ZIBaseModule(ZIInstrument):
    ...
    def _set_node(signal: t.Union[ZIParameter, TKNode, str]) -> str:
        ...
        try:
            ...
            node = TKBaseModule._set_node(signal)  # type: ignore[arg-type]
    ...
    def subscribe(self, signal: t.Union[ZIParameter, str]):
        ...
        """
        tk_module = t.cast(TKBaseModule, self._tk_object)
        try:
            tk_module.subscribe(signal.zi_node)  # type: ignore[union-attr]
        except AttributeError:
            tk_module.subscribe(signal)  # type: ignore[arg-type]
    ...
    def unsubscribe(self, signal: t.Union[ZIParameter, str]):
        ...
        tk_module = t.cast(TKBaseModule, self._tk_object)
        try:
            tk_module.unsubscribe(signal.zi_node)  # type: ignore[union-attr]
        except AttributeError:
            tk_module.unsubscribe(signal)  # type: ignore[arg-type]

    @property
    def raw_module(self) -> ZIModule:  # type: ignore[type-var]
        """Underlying zhinst.core module."""
        return self._tk_object.raw_module  # type: ignore[return-value, union-attr]
